        ...
        <Container maxWidth='lg'>
          <Toolbar>
            ...
            <Box style={{ display: 'flex', alignItems: 'center', textAlign: 'center', margin: '0px 25px'  }}>
              {NavItems.map((it) => {
                return <Typography variant='sm' style={{ margin: '10px', padding:'14px 0px', color:currentNav === it.name?'white':'rgba(239,241,246,.75)', fontWeight:currentNav === it.name?'900px':'100px', borderBottom:currentNav === it.name?'2px solid white':'none', borderRadius:'2px', borderBottomWidth:3}}>{it.name}</Typography>
              })}
            </Box>
          </Toolbar>
        </Container>
